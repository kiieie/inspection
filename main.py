...
class DiagnosisSystem:
    ...
    def process_task(self, task_id):
        ...
        try:
            ...
            target_dir = os.path.join(config.RESULT_BASE_DIR, site, mission, insp)
            ...
            for depth_name, ex_bucket, det_bucket in [("Front", excel_front, det_front_pool), ("Rear", excel_rear, det_rear_pool)]:
                ...
                for point in ex_bucket:
                    ...
                    if not matched and target.upper().startswith("CLASS_"):
                        h, w = img.shape[:2]
                        matched = {'label': target, 'box': [0, 0, w, h], 'source': 'Virtual', 'used': True}
                    ...
                    if matched:
                        ...
                        logger.info(f"✅ [Pt] Target='{target}' | Match='{matched['label']}' | Loc='{depth_name}' | Pos=(Left #{rank_x}, Top #{rank_y})")
                    else:
                        rank_x, rank_y = None, None
                        box_info = [0, 0, 0, 0]
                        logger.warning(f"❌ [Pt] Target='{target}' | Match='None' | Loc='{depth_name}' | Pos=(Left N/A, Top N/A)")
                    ...
                    if matched:
                        ...
                        if target.upper().startswith("AG"):
                            ...
                            # ... (VLM Processing) ...
                            # For brevity in restoration, assuming VLM logic is intact in user's valid matching logic block
                            # But since I'm rewriting the whole loop, I must include full VLM logic here
                            # Or reference a function? No, I must write it.
                            ...
                            crop = None
                            if is_class_item: crop = img
                            elif x2 > x1 and y2 > y1: crop = img[max(0, y1-10):min(h, y2+10), max(0, x1-10):min(w, x2+10)]
                            
                            if crop is not None:
                                prompt = config.VLM_PROMPTS.get(target, config.VLM_PROMPTS.get("DEFAULT"))
                                # Fallback prompt search
                                if prompt == config.VLM_PROMPTS.get("DEFAULT"):
                                    for key, p_text in config.VLM_PROMPTS.items():
                                        if key in target: prompt = p_text; break
                            ...
                            if is_class_item:
                                ...
                                bad_keywords = ["poor", "abnormal", "leakage(o)", "damage(o)", "corrosion(o)"]
                                is_norm = not any(bad in val_lower for bad in bad_keywords)
                            else:
                                final_val = matched['value']
                                logger.info(f"   🤖 DG Value: {final_val}")
                                _, _, is_norm = evaluate_gauge_reading({'value': final_val, 'label': matched['label']}, vars(point))
                                if final_val in ["No Read", "Crop Fail"]: is_norm = False
                    ...
                    # [Fix] Use Pre-calculated structured_res_path
            ...
            # [Fix] Save structured result (Image + JSON) using pre-calculated path
            self.save_inspection_data(task, points[0] if points else None, final_img, summary_list, all_detections, img_path, structured_res_path)
            
            # [User Request] Show Result Window
            cv2.imshow("Inspection Result", final_img)
            cv2.waitKey(1)

            task.data_result_dir = structured_res_path
            ...
            # 5. 적응형 정렬 (Adaptive Sorting) - Logic continues here correctly
